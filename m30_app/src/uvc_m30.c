...
#define FRAME_POLL_US             5000      /* 帧轮询间隔 5ms，匹配 25fps/40ms 周期 */
#define FRAME_POLL_MAX            200       /* 最多轮询 200 次 ≈ 1 秒超时 */
...
int m30_set_resolution(m30_ctx_t *ctx, m30_resolution_t mode)
{
    if (!ctx) return M30_ERR_INVAL;
    LOCK(ctx); REQUIRE_OPEN_UNLOCK(ctx);
    int rc = SetResolution(ctx->sdk_dev, (char)mode);
    /* 分辨率变更后清除缓存，下次 GetFrame 会重新查询 */
    ctx->cached_w = 0;
    ctx->cached_h = 0;
    RETURN_UNLOCK(ctx, map_sdk_err(rc));
}
...
/* 刷新缓存分辨率，仅在首次或分辨率变更时调用 SDK */
static int refresh_resolution(m30_ctx_t *ctx)
{
    int w = 0, h = 0;
    int rc = GetResolution(ctx->sdk_dev, &w, &h);
    if (rc != 0 || w <= 0 || h <= 0) {
        UVC_LOGE("GetResolution failed rc=%d w=%d h=%d", rc, w, h);
        return rc ? rc : AC_ERR_OUTPUT_ARG;
    }
    ctx->cached_w = w;
    ctx->cached_h = h;
    return 0;
...
int m30_get_frame(m30_ctx_t *ctx, m30_frame_t *out)
{
    ...
    /* 首次拉帧时查询一次分辨率并缓存 */
    if (ctx->cached_w <= 0 || ctx->cached_h <= 0) {
        int rc = refresh_resolution(ctx);
        if (rc != 0) RETURN_UNLOCK(ctx, map_sdk_err(rc));
    }

    const int w = ctx->cached_w;
    const int h = ctx->cached_h;
    ...
    if (rc != AC_ERR_SUCCESS) {
        UVC_LOGE("GetFrame timeout after %d polls", attempts);
...
int m30_save_frame_bmp(m30_ctx_t *ctx, const char *path)
{
    ...
    UVC_LOGI("frame saved: %s (%dx%d)", path, fr.width, fr.height);
    return M30_OK;
}
