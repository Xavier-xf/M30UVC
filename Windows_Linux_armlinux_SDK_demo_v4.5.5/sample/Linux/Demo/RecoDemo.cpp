...
void getReoImageHandle(char * value, int len) 
{
    if (len > 4)
    {
		printf("Recv ReoImageData\n");
	}
}
...
int SaveFrame(void* dev, const char* savePath)
{	
	int width = 0;
	int height = 0;	
	// printf("error:%s,%d\n",__FILE__,__LINE__);
	...
	sleep(5);
	if (GetFrame(dev, imageBuf, imageSize, verifyBuf, sizeof(verifyBuf)))	
		return -1;
	printf("GetFrame verifyBuf : %s\n", verifyBuf);
	//帧数据保存为帧图片
	FILE *file = fopen(savePath, "w+");
	writeBMP(savePath, reinterpret_cast<uint32_t *>(imageBuf), width, height);
...
{	
    ...
    /* Initialize M20 SDK */
	int ret1 = SetLogConfig(1, 2); // 保存debug日志
	if(ret1 != 0)
	{
	...
	while(!isStop)
	{
		...
		switch(cmdNum)
		{
			...
			{	
				...
				{
					int ret2 = OpenAutoUploadAiInfo(dev, 0, 0);					 // M20 SDK API
					printf("OpenAutoUploadAiInfo done ret %d\n", ret2);

					ret2 = StartOnetoNumRecognize(dev, nRecMode, nMulMode);     // M20 SDK API
					printf("StartOnetoNumRecognize done ret %d\n", ret2);
